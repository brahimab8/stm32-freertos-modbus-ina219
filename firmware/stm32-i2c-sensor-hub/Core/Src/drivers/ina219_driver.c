/* Auto-generated ina219_driver.c */
...
static halif_status_t ini(void *ctx) {
    ...
    /* compute calibration from dependencies */
    c->calibration = ((uint16_t)(0.04096f / (((float)c->current_lsb_uA / 1e6f) * ((float)c->shunt_milliohm / 1000.0f)) + 0.5f));
    if (INA219_SetCalibration(c->h_i2c, c->addr7, c->calibration) != HALIF_OK) return HALIF_ERROR;
    c->payload_mask = INA219_DEFAULT_PAYLOAD_MASK;  /* default mask */
    return HALIF_OK;
}
...
static halif_status_t rd(void *ctx, uint8_t out_buf[], uint8_t *out_len) {
    ...
    if (mask & INA219_PAYLOAD_BIT_BUS_VOLTAGE_MV) {
        if ((cursor - out_buf + 2) > SENSOR_MAX_PAYLOAD) {
            *out_len = 0;
            return HALIF_ERROR;
        }
        ...
        /* pack big-endian */
        for (int b = 2 - 1; b >= 0; --b) {
            *cursor++ = (uint8_t)((var_bus_voltage_mV >> (8 * b)) & 0xFF);
        }
        total_bytes += 2;
    }
    if (mask & INA219_PAYLOAD_BIT_SHUNT_VOLTAGE_UV) {
        if ((cursor - out_buf + 2) > SENSOR_MAX_PAYLOAD) {
            *out_len = 0;
            return HALIF_ERROR;
        }
        ...
        /* pack big-endian */
        for (int b = 2 - 1; b >= 0; --b) {
            *cursor++ = (uint8_t)((var_shunt_voltage_uV >> (8 * b)) & 0xFF);
        }
        total_bytes += 2;
    }
    if (mask & INA219_PAYLOAD_BIT_CURRENT_UA) {
        if ((cursor - out_buf + 2) > SENSOR_MAX_PAYLOAD) {
            *out_len = 0;
            return HALIF_ERROR;
        }
        ...
        /* pack big-endian */
        for (int b = 2 - 1; b >= 0; --b) {
            *cursor++ = (uint8_t)((var_current_uA >> (8 * b)) & 0xFF);
        }
        total_bytes += 2;
    }
    if (mask & INA219_PAYLOAD_BIT_POWER_MW) {
        if ((cursor - out_buf + 2) > SENSOR_MAX_PAYLOAD) {
            *out_len = 0;
            return HALIF_ERROR;
        }
        ...
        /* pack big-endian */
        for (int b = 2 - 1; b >= 0; --b) {
            *cursor++ = (uint8_t)((var_power_mW >> (8 * b)) & 0xFF);
        }
        total_bytes += 2;
    }
    *out_len = total_bytes;
...
bool ina219_read_config_bytes(void *vctx, uint8_t field, uint8_t *out_buf, size_t *out_len) {
    ...
    switch (field) {
      ...
      case CMD_GET_CAL:
      {
          /* read computed field calibration from the device */
          INA219_CALIBRATION_t val;
          if (INA219_ReadCalibration(c->h_i2c, c->addr7, &val) != HALIF_OK) {
              return false;
          }
          /* pack into big-endian bytes */
          for (int i = 0; i < 2; ++i) {
              out_buf[i] = (uint8_t)( val >> (8 * (2 - 1 - i)) );
          }
          *out_len = 2;
          return true;
      }
...
static uint8_t get_sample_size(void *ctx) {
    INA219_Ctx_t *c = (INA219_Ctx_t *)ctx;
    uint8_t size = 0;
    if (c->payload_mask & INA219_PAYLOAD_BIT_BUS_VOLTAGE_MV) {
        size += 2;
    }
    if (c->payload_mask & INA219_PAYLOAD_BIT_SHUNT_VOLTAGE_UV) {
        size += 2;
    }
    if (c->payload_mask & INA219_PAYLOAD_BIT_CURRENT_UA) {
        size += 2;
    }
    if (c->payload_mask & INA219_PAYLOAD_BIT_POWER_MW) {
        size += 2;
    }
    return size;
}
...
static const SensorDriverInfo_t ina219_info = {
    ...
    .get_default_period_ms = ina219_default_period_ms,
};
