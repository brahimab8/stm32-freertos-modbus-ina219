...
halif_status_t INA219_SetBusRange(
    halif_handle_t   h_i2c, uint8_t addr7bit, INA219_BUS_RANGE_t value) {
    uint8_t buf[2] = { 0x00, (uint8_t)value };
    return halif_i2c_write(h_i2c, addr7bit, buf, 2, 100);
}
...
halif_status_t INA219_ReadGain(
    halif_handle_t   h_i2c, uint8_t addr7bit, uint8_t *out) {
    uint8_t cmd = 0x00;
    uint8_t data[1];
    if (halif_i2c_write(h_i2c, addr7bit, &cmd, 1, 100) != HALIF_OK) return HALIF_ERROR;
    if (halif_i2c_read(h_i2c, addr7bit, data, 1, 100) != HALIF_OK) return HALIF_ERROR;
    *out = data[0];
    return HALIF_OK;
}
...
halif_status_t INA219_ReadCalibration(
    halif_handle_t   h_i2c, uint8_t addr7bit, uint16_t *out) {
    uint8_t cmd = 0x05;
    uint8_t data[2];
    if (halif_i2c_write(h_i2c, addr7bit, &cmd, 1, 100) != HALIF_OK) return HALIF_ERROR;
    if (halif_i2c_read(h_i2c, addr7bit, data, 2, 100) != HALIF_OK) return HALIF_ERROR;
...
halif_status_t INA219_ReadShuntVoltageUv(
    halif_handle_t   h_i2c, uint8_t addr7bit, int16_t *out) {
